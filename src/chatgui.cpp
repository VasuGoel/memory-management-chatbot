...
bool ChatBotApp::OnInit()
{
    ...
    ChatBotFrame *chatBotFrame = new ChatBotFrame(wxT("ChatBot - by Vasu"));
...
ChatBotFrame::ChatBotFrame(const wxString &title) : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(width, height))
{
    ...
    _userTextCtrl->SetBackgroundColour(wxColour(0xd3,0xd3,0xd3));
...
void ChatBotFrameImagePanel::render(wxDC &dc)
{
    ...
    wxString imgFile = imgBasePath + "white.jpg";
    wxImage image;
...
{
    wxImage image;
    image.LoadFile(imgBasePath + "white-inner.jpg");
    ...
    : wxPanel(parent, -1, wxPoint(-1, -1), wxSize(-1, -1), wxBORDER_NONE)
{
    ...
    this->SetBackgroundColour((isFromUser == true ? wxT("GREEN") : wxT("BLUE")));
}